...
// }
// // ──────────────────────────────────────────────
// // SHOPIFY-STYLE EMAIL TEMPLATE
// // ──────────────────────────────────────────────

// /**
//  * Shopify SVG bag logo (inline, email-safe).
//  * Uses Shopify's official icon shape with customizable fill color.
//  */
// function shopifyBagSvg(color = '#96BF48') {
//   return `<img src="https://cdn.shopify.com/shopifycloud/shopify/assets/shopify_favicon-8f0f76287e009a2e90c4c43e358bcf2c6ef1dbf4a617e99c45a62c5e66e6b429.png" width="20" height="20" alt="" style="display:inline-block;vertical-align:middle;margin-right:6px;" />`;
// }
...
//   <title>New message from ${esc(brandName)}</title>
// </head>
...
//                       You received this email because you have an active conversation with
//                       <a href="${replyUrl}" style="color:#8c9196; text-decoration:underline;">${esc(storeDomain)}</a>.
//                     </p>
//                     <p style="margin:0; font-size:12px; color:#8c9196;">
//                       You won't receive another notification for at least ${COOLDOWN_HOURS} hour${COOLDOWN_HOURS > 1 ? 's' : ''}.
//                     </p>
//                   </td>
//                 </tr>
...
//                       &copy; ${year} ${esc(brandName)} &middot;
//                       <a href="https://${esc(storeDomain)}" style="color:#babec3; text-decoration:underline;">${esc(storeDomain)}</a>
//                     </p>
//                   </td>
//                 </tr>
...
async function sendOfflineEmail(pool, conversation_id, triggerMessageId) {
  try {
    ...
    // Use last emailed message timestamp as cutoff
    // NOTE: We intentionally do NOT override with last_activity_at here,
    // as that was causing new agent messages to be skipped.
    let sinceTimestamp = '1970-01-01T00:00:00Z';
    if (lastEmailedMsg.rows.length) {
      sinceTimestamp = lastEmailedMsg.rows[0].last_msg_timestamp;
    }
    ...
    if (!unreadMessages.length) {
      console.log(`[Email] No unread messages for conv ${conversation_id}, skipping`);
      return;
    }
    ...
    // ── Send via fetch (no Resend SDK) ──────────────────────
    const apiKey = process.env.RESEND_API_KEY;
    console.log('[Email] API key present:', !!apiKey, '| length:', apiKey?.length);
    ...
    console.log('[Email] Resend HTTP status:', resendRes.status, JSON.stringify(resendBody));

    if (!resendRes.ok) {
      console.error('[Email] Resend error:', resendBody);
      return;
    }
