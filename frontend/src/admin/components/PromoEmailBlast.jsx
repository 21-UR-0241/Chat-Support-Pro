...
// // ── Suppression model ─────────────────────────────────────────────────────────
// // The DB enforces ONE promo per (email, store_domain) via the unique index
// // promo_sent_emails_email_store_uidx on (LOWER(email), store_domain). There is
// // no campaign column — getPromoRecipients excludes anyone already recorded for
// // their purchase store, plus anyone in promo_unsubscribes / blacklisted /
// // archived. To re-send to a whole store, clear that store's rows in
// // promo_sent_emails; every purchaser then becomes eligible again on the next
// // reload. (Unsubscribes are still honoured regardless.)
...
// // ── Email copy ────────────────────────────────────────────────────────────────
// const DEFAULT_SUBJECT = "We've moved to montrealpeptides.com";

// const DEFAULT_BODY =
// `Hi there,

// We're writing to let you know that Montreal Peptides has moved to a new home online. We're now at montrealpeptides.com (previously montrealpeptides.ca).

// This change is part of ongoing upgrades to our platform — improved service, faster performance, and tightened security to keep your information and orders better protected.

// What this means for you:
// - Please update any saved bookmarks from montrealpeptides.ca to montrealpeptides.com
// - Your account, order history, and details remain unchanged
// - All future orders and communications will come from our new domain, montrealpeptides.com

// We're committed to continually improving our services and giving you the best possible experience. If you need anything, our support team is always available through the chat widget on our homepage at montrealpeptides.com.

// Thank you for your continued trust.

// Warm regards,
// Montreal Peptides Team`;
...
//       const raw  = (await api.getPromoRecipients({ storeIds })) || [];
//       const rows = dedupeByEmail(raw.map(normalizeRecipient).filter((r) => r.email));
//       console.log('[Promo] raw:', raw.length, '→ after email filter+dedup:', rows.length);
//       console.log('[Promo] by store:',
//         Object.entries(rows.reduce((a, r) => {
//           const k = `${r.storeName || '?'} · ${r.storeDomain || r.storeId || '?'}`;
...
//       .join(cardHtml)
//       .replace(
//         /montrealpeptides\.com/g,
//         '<a href="https://montrealpeptides.com" style="color:#4f46e5;text-decoration:underline;font-weight:600;">montrealpeptides.com</a>'
//       )
//       .replace(
//         /\{\{store_url\}\}/g,
//         '<a href="{{store_url}}" style="color:#4f46e5;text-decoration:none;font-weight:600;">{{store_url}}</a>'
...
//             {recipients.length > 0 && (
//               <div className="peb-muted peb-mt8">
//                 {selected.length} of {recipients.length} selected (already-sent customers excluded by DB)
//               </div>
//             )}
...
//                   <p>No new recipients found.</p>
//                   <p className="peb-muted">
//                     Everyone eligible has already received a promo for their store, or no purchasers
//                     exist for this store selection. To re-send, clear the store's rows in promo_sent_emails.
//                   </p>
//                 </div>
//               ) : (
//                 <>
...
//                   <div className="peb-muted peb-mt8">
//                     Showing {filtered.length} of {recipients.length}.
//                     Customers who already received a promo for their purchase store are excluded automatically by the database.
//                     Duplicate emails merged to one (most recent purchase store).
//                   </div>
//                 </>
//               )}
