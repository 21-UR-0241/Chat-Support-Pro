...
//       if (!res.ok) {
//         // Credit exhaustion is not transient — don't waste retries, fall back immediately.
//         if (isCreditExhaustedError(text)) {
//           console.warn('[AI] Anthropic credits exhausted — attempting DeepSeek fallback');
//           const fallback = await tryDeepSeekFallback(requestBody);
...
//         if (RETRYABLE_STATUS.includes(res.status) && isLastAttempt) {
//           console.warn(`[AI] Anthropic ${res.status} after ${MAX_ATTEMPTS} attempts — attempting DeepSeek fallback`);
//           const fallback = await tryDeepSeekFallback(requestBody);
//           if (fallback) return fallback;
//         }
