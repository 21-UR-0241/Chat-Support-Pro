...
(function (global) {
  ...
  // ── Delivery resilience ────────────────────────────────────────────────────
  // The agent's replies reach this widget ONLY over the WebSocket: the customer
  // sends over REST (so the agent always receives), but sendToConversation on
  // the server is the single inbound path here. That made a dead socket look
  // like a dead conversation — messages appeared only after a page reload.
  //
  // Two changes address it: the socket now retries forever with backoff instead
  // of giving up after a fixed number of attempts, and a REST poll runs
  // underneath as a floor so delivery never depends on the socket alone.
  var MAX_RECONNECT_DELAY = 30 * 1000; // backoff ceiling — we never stop trying
  var POLL_INTERVAL       = 5 * 1000;  // poll tick, and the rate when WS is down
  var WS_VERIFY_EVERY     = 30 * 1000; // reconciliation fetch while WS looks healthy
  ...
  function createChatCore(opts) {
    ...
    var customerEmail = null, customerName = null, customerData = null;
    var customerOrders = [], customerCart = null, storeSettings = null;
    var reconnectAttempts = 0, reconnectDelay = 2000, reconnectTimer = null;
    var displayedMessageIds = new Set(), historyLoaded = false, messageSource = null;
    var uploading = false;
    var lastActivityTime = Date.now(), isPageVisible = true, isCustomerActive = true;
    var heartbeatTimer = null, presenceApiTimer = null, activityThrottleTimer = null;
    // Delivery-health state. `wsJoined` matters because the server routes
    // conversation traffic by the socket's joined conversation, so an
    // authenticated-but-unjoined socket receives nothing.
    var wsJoined = false, wsTrusted = true, pollTimer = null, lastVerifyAt = 0;
    ...
    async function fetchCustomerData(email) {
      ...
      try {
        var r = await fetchWithTimeout(API_URL + '/api/customers/lookup?store=' + STORE_ID + '&email=' + encodeURIComponent(email), {}, 12000);
        // 404 just means this email has never ordered — an ordinary outcome for
        // a new customer, not an error worth logging to the console.
        if (r.ok) { customerData = await r.json(); return customerData; }
      } catch (e) { console.warn('[Customer] Lookup unavailable:', e.message); }
      return null;
    }
    async function fetchCustomerOrders(email) {
      if (!email) return [];
      try {
        var r = await fetchWithTimeout(API_URL + '/api/customers/orders?store=' + STORE_ID + '&email=' + encodeURIComponent(email), {}, 12000);
        if (r.ok) { customerOrders = await r.json(); return customerOrders; }
      } catch (e) { console.warn('[Customer] Orders unavailable:', e.message); }
      return [];
    }
    async function fetchCustomerCart(email) {
      if (!email) return null;
      ...
      try {
        var r = await fetchWithTimeout(API_URL + '/api/customers/cart?store=' + STORE_ID + '&email=' + encodeURIComponent(email), {}, 12000);
        if (r.ok) { customerCart = await r.json(); return customerCart; }
      } catch (e) { console.warn('[Customer] Cart unavailable:', e.message); }
      return null;
    }
    async function loadCustomerData() {
      if (!customerEmail) return;
      try { await Promise.all([fetchCustomerData(customerEmail), fetchCustomerOrders(customerEmail), fetchCustomerCart(customerEmail)]); }
      catch (e) { console.warn('[Customer] Context load incomplete:', e.message); }
    }
    async function lookupConversation(email) {
      if (!email || !STORE_ID) return null;
      try {
    ...
    // ---- message sync (REST floor under the socket) ----------------------
    // Fetches the thread and emits anything not yet seen. This is what makes
    // agent replies arrive when the socket is down, half-open, or never joined.
    // Own (customer) messages are recorded but not re-emitted: the view already
    // rendered them optimistically at send time.
    async function syncMessages() {
      if (!conversationId || !STORE_ID) return 0;
      var url = API_URL + '/api/widget/conversations/' + conversationId + '/messages?store=' + STORE_ID;
      var r;
      try { r = await fetchWithTimeout(url, { headers: { 'Content-Type': 'application/json' } }, 15000); }
      catch (e) { return 0; }
      if (!r || !r.ok) {
        if (r && r.status === 404) {
          // Conversation is gone server-side — drop the stale id so the next
          // send starts a fresh one instead of failing forever.
          localStorage.removeItem('chat_conv_' + STORE_ID);
          conversationId = null;
          stopMessagePolling();
        }
      ...
      messages.forEach(function (m) {
        var n = normalize(m);
        if (n.id == null || displayedMessageIds.has(n.id)) return;
        displayedMessageIds.add(n.id);
        if (n.senderType === 'customer') return; // our own echo
        delivered++;
        emit('typing', false);
        emit('message', n);
      });
    ...
    function startMessagePolling() {
      ...
      pollTimer = setInterval(async function () {
        if (!conversationId) return;
        // "Live" means OPEN *and* joined — an authenticated socket that never
        // joined receives no conversation traffic at all.
        var live = !!(ws && ws.readyState === WebSocket.OPEN && wsJoined && wsTrusted);
        if (live && (Date.now() - lastVerifyAt) < WS_VERIFY_EVERY) return;
        lastVerifyAt = Date.now();
        var delivered = await syncMessages();
        if (delivered > 0 && live) {
          // The socket says it's healthy but the messages came from the poll.
          // Stop trusting it and poll at full rate until a frame proves
          // otherwise, then rebuild the connection.
          console.warn('[ChatCore] ' + delivered + ' message(s) arrived by poll, not socket — reconnecting');
          wsTrusted = false;
          reconnectNow('socket connected but not delivering');
        }
      }, POLL_INTERVAL);
    }
    ...
    function startActivityTracking() {
      ...
      global.document.addEventListener('visibilitychange', function () {
        isPageVisible = !global.document.hidden;
        if (isPageVisible) { onUserActivity(); handleNetworkBack(); }
        else { isCustomerActive = false; sendPresenceUpdate('away'); }
      });
      global.addEventListener('focus', function () { isPageVisible = true; onUserActivity(); handleNetworkBack(); });
      global.addEventListener('blur', function () { isPageVisible = false; isCustomerActive = false; sendPresenceUpdate('away'); });
      heartbeatTimer = setInterval(function () {
        if (Date.now() - lastActivityTime >= INACTIVE_THRESHOLD && isCustomerActive) { isCustomerActive = false; sendPresenceUpdate('away'); }
        sendPresenceHeartbeat();
    ...
    function stopActivityTracking() {
      ...
      if (presenceApiTimer) clearInterval(presenceApiTimer);
    }
    function sendPresenceUpdate(status) {
      if (!conversationId) return;
      if (ws && ws.readyState === WebSocket.OPEN) {
    ...
    function sendPresenceViaApi(statusOverride) {
      ...
      fetch(API_URL + '/api/widget/presence', {
        ...
        if (res.status === 410) { stopActivityTracking(); stopMessagePolling(); conversationId = null; localStorage.removeItem('chat_conv_' + STORE_ID); }
      }).catch(function (err) { console.warn('[Presence API] Failed:', err.message); });
    }
    global.addEventListener('beforeunload', function () {
      if (conversationId && customerEmail) {
        ...
        if (navigator.sendBeacon) navigator.sendBeacon(API_URL + '/api/widget/presence', new Blob([payload], { type: 'application/json' }));
        if (ws && ws.readyState === WebSocket.OPEN) {
          ws.send(JSON.stringify({ type: 'presence', conversationId: parseInt(conversationId), status: 'offline', customerEmail: customerEmail }));
          teardownSocket();
        }
      }
    });
    ...
    // Detach every handler BEFORE closing. Without this, closing the old socket
    // fires its own onclose, which schedules a reconnect and increments the
    // attempt counter — the connection was effectively racing itself and
    // burning through its own retry budget.
    ...
    function scheduleReconnect() {
      ...
      reconnectAttempts++;
      // Exponential backoff with a ceiling, and NO give-up point. The previous
      // version stopped after five attempts and never tried again — one Render
      // cold start was enough to leave the widget permanently offline, with
      // agent replies only appearing on a page reload.
      var delay = Math.min(reconnectDelay * Math.pow(2, reconnectAttempts - 1), MAX_RECONNECT_DELAY);
    ...
    function reconnectNow(reason) {
      if (reason) console.warn('[ChatCore] Forcing reconnect:', reason);
    ...
    // Refocus / network-return recovery. Pulls anything missed while the tab was
    // throttled, and revives a socket that is closed or half-open.
    function handleNetworkBack() {
      if (!conversationId) return;
      syncMessages();
      if (ws && ws.readyState === WebSocket.OPEN) {
        // Open but never joined can't receive conversation traffic — rebuild it.
        if (!wsJoined) reconnectNow('socket open but not joined');
        return;
      }
      if (ws && ws.readyState === WebSocket.CONNECTING) return;
      reconnectNow('network or visibility regained');
    }
    global.addEventListener('online', function () { handleNetworkBack(); });
    ...
    async function connectWebSocket() {
      ...
      try {
        emit('status', { state: 'connecting', text: 'Connecting...' });
        await ensureFreshToken(reconnectAttempts > 0);
        if (!wsToken) { emit('status', { state: 'offline', text: 'No token' }); scheduleReconnect(); return; }
        teardownSocket();
        ...
        sock.onopen = function () {
          reconnectAttempts = 0;
          wsTrusted = true;
          sock.send(JSON.stringify({ type: 'auth', token: wsToken, clientType: 'customer' }));
        };
        sock.onmessage = function (event) {
          try { handleWebSocketMessage(JSON.parse(event.data)); }
          catch (e) { console.error('[WebSocket] Parse error:', e); }
        };
        sock.onerror = function () { emit('status', { state: 'offline', text: 'Error' }); };
        sock.onclose = function () {
          if (ws !== sock) return; // superseded by a newer socket — ignore
          ws = null;
          wsJoined = false;
          emit('status', { state: 'offline', text: 'Disconnected' });
          // Pull anything that landed while the socket was going down, then
          // keep retrying. The poll continues underneath either way.
          syncMessages();
          scheduleReconnect();
        };
      } catch (e) {
        console.error('[WebSocket] Failed:', e);
        emit('status', { state: 'offline', text: 'Failed' });
        scheduleReconnect();
      }
    }
    ...
    function handleWebSocketMessage(data) {
      switch (data.type) {
        ...
        case 'auth_ok':
          ...
          break;
        case 'joined':
          // Only now can this socket receive conversation traffic.
          wsJoined = true;
          wsTrusted = true;
          lastVerifyAt = Date.now();
          emit('status', { state: 'online', text: GREET });
          ...
          syncMessages();
          break;
        case 'new_message': if (data.message) { wsTrusted = true; onUserActivity(); routeIncoming(data.message); } break;
        case 'message_confirmed':
          wsTrusted = true;
          if (data.message && data.message.id) displayedMessageIds.add(data.message.id);
          emit('message:update', { tempId: data.tempId, status: 'confirmed', id: data.message && data.message.id });
          break;
        case 'message_failed': emit('message:update', { tempId: data.tempId, status: 'failed' }); break;
        case 'typing': if (data.isTyping && data.senderType === 'agent') emit('typing', true); else if (!data.isTyping) emit('typing', false); break;
        case 'message_deleted': if (data.messageId) { displayedMessageIds.delete(data.messageId); emit('message:delete', data.messageId); } break;
    ...
    function routeIncoming(message) {
      var msg = normalize(message);
      // Compare as strings: conversationId is held as a string (localStorage,
      // String(lu)) while the server sends a number, and parseInt() on a
      // non-numeric value yields NaN, which fails every comparison silently.
      if (msg.conversationId != null && String(msg.conversationId) !== String(conversationId)) return;
      if (msg.id != null && displayedMessageIds.has(msg.id)) return;
      // our own customer message echoed back — just record id, view already shows it
      if (msg.senderType === 'customer') { if (msg.id != null) displayedMessageIds.add(msg.id); return; }
      emit('typing', false);
      if (msg.id != null) displayedMessageIds.add(msg.id);
      emit('message', msg);
    }
    ...
    async function send(text, file) {
      ...
      try {
        if (!conversationId) {
          ...
          emit('message:update', { tempId: tempId, status: 'confirmed', id: tempId });
          sendPresenceUpdate('online');
          // The conversation now exists, so both the socket and the poll have
          // something to attach to. Previously the socket was only started when
          // a token happened to be cached, and nothing polled at all.
          startMessagePolling();
          reconnectNow('conversation created');
          var ar = 'Thanks for reaching out! An agent will be with you shortly.';
          if (customerData && customerData.name) ar = 'Hi ' + customerData.name + '! ' + ar;
          if (customerOrders.length > 0) ar += ' We can see you have ' + customerOrders.length + ' order' + (customerOrders.length !== 1 ? 's' : '') + ' with us.';
          emit('message', { id: 'auto-' + tempId, senderType: 'agent', senderName: 'Support Team', content: ar, timestamp: new Date().toISOString() });
    ...
    async function enterChat() {
      ...
      sendPresenceUpdate('online');
      if (conversationId) {
        // Poll first, socket second. The poll is the floor: it guarantees the
        // customer sees agent replies even if the socket never comes up.
        startMessagePolling();
        emit('status', { state: 'connecting', text: 'Connecting...' });
        try {
          if (!wsToken) await ensureFreshToken(true);
          if (wsToken) connectWebSocket();
          else emit('status', { state: 'online', text: GREET });
        } catch (e) { emit('status', { state: 'online', text: GREET }); }
      } else emit('status', { state: 'online', text: GREET });
    }
