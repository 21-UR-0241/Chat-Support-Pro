...
//   // ── Shared keyframe ───────────────────────────────────────
//   var IC_RING = '@keyframes cw-ic-ring{'
//     + '0%{box-shadow:0 0 0 0 '+hexToRgba(T.primary, .55)+'}'
...
//     // 0 — Gradient pill (no icon badge, no pulse)
//     {
//       css: [
...
//     // 1 — Soft pill with pulsing icon circle
//     {
//       css: [
//         IC_RING,
...
//     // 3 — White pill with colored left block (no icon badge pulse)
//     {
//       css: [
//         '#cwb{position:fixed;bottom:24px;left:24px;height:54px;border-radius:27px;',
...
//     // 4 — Pill with pulsing icon circle (no button-level pulse)
//     {
//       css: [
//         IC_RING,
//         '#cwb{position:fixed;bottom:24px;left:24px;height:50px;border-radius:25px;',
...
//     // 5 — Gradient pill with pulsing frosted icon badge
//     {
//       css: [
//         IC_RING,
...
//     // 7 — Avatar initials pill (no icon badge, no pulse)
//     {
//       css: [
...
//     // 8 — Gradient pill with pulsing smiley icon circle (fixed: ic.s8)
//     {
//       css: [
//         IC_RING,
//         '#cwb{position:fixed;bottom:24px;left:24px;height:52px;border-radius:26px;',
...
//       html: '<button id="cwb" aria-label="Chat with us">'
//           + '<span style="display:flex;align-items:center;">'
//           + '<span class="cw-ic">'+ic.s8+'</span>'
//           + '<span class="cw-lbl">Chat</span>'
//           + '</span>'
//           + '</button>'
//     },

//     // 9 — White rounded card with top accent strip (no icon badge, no pulse)
//     {
//       css: [
...
(function() {
  ...
  // Hidden-when-open rule — applied to every style
  var TCSS = '#cwb.active{opacity:.9;}#cwb.cw-hidden{display:none!important;}';
  var FF   = 'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;';
  ...
  var iframeCSS = [
    ...
    '@media(max-width:480px){',
    '#chat-widget-iframe,#chat-widget-iframe.open{',
    'width:100vw;height:100vh;height:100dvh;bottom:0;left:0;border-radius:0;transform:none;}',
    '#cwb{bottom:16px!important;left:16px!important;}}'
  ].join('');
  ...
  function openWidget() {
    ...
    requestAnimationFrame(function() { frame.classList.add('open'); });
    btn.classList.add('active');
    btn.classList.add('cw-hidden');   // hide FAB while chat is open
  }
  ...
  function closeWidget() {
    ...
    frame.classList.remove('open');
    btn.classList.remove('active');
    btn.classList.remove('cw-hidden'); // show FAB again
    setTimeout(function() { if (!isOpen) frame.style.display = 'none'; }, 280);
  }
  ...
  // Listen for close request from inside the iframe (the X button in widget header)
  window.addEventListener('message', function(e) {
    if (!e.data || typeof e.data !== 'object') return;
    if (e.data.type === 'chat-widget-close') closeWidget();
    else if (e.data.type === 'chat-widget-open') openWidget();
  });
