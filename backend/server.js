...
function stripStoreSecrets(store) {
  ...
  const safe = { ...store };
  for (const field of STORE_SECRET_FIELDS) delete safe[field];
  return safe;
}
...
async function getRequestStoreScope(req) {
  ...
  const scope = await db.getEmployeeStoreScope(req.user.id);
  appCache.set(cacheKey, scope, TTL.SCOPE);
  return scope;
}
...
/** SQL fragment + params for scoping a raw conversations query by group. */
function scopeSqlClause(scope, params, alias = 'c') {
  if (scope.canViewAll) return '';
  params.push(scope.groups);
  return ` AND ${alias}.shop_id IN (SELECT id FROM stores WHERE store_group = ANY($${params.length}::text[]))`;
}
...
app.put('/api/stores/groups/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Renaming the key would orphan every employee assigned the old slug —
    // they'd keep a group that no longer matches any store and see nothing.
    const oldKey = req.params.id;
    if (groupKey.trim() !== oldKey) {
      await db.pool.query(
        `UPDATE employees
            SET assigned_groups = array_replace(assigned_groups, $1::text, $2::text),
                updated_at = NOW()
          WHERE $1::text = ANY(assigned_groups)`, [oldKey, groupKey.trim()]).catch(() => {});
      appCache.invalidate('employees:list');
      appCache.invalidatePrefix('scope:');
    }
...
app.post('/api/email/send', authenticateToken, async (req, res) => {
  ...
  if (!to || !subject || !body) return res.status(400).json({ error: 'to, subject, and body are required' });
  try {
    if (conversationId && !(await canAccessConversation(req, parseInt(conversationId, 10)))) {
...
// Feeds the store filter AND the employee store-assignment picker. Rows are
// stripped of Shopify credentials before they leave the process — see
// stripStoreSecrets. Restricted agents only ever see their assigned stores, or
// the picker/filter would advertise stores they cannot open.
app.get('/api/stores', authenticateToken, async (req, res) => {
  try {
    const { storeGroup } = req.query;
    const scope = await getRequestStoreScope(req);
    const scopeKey = scopeCacheKey(scope, req);
    const cacheKey = storeGroup
      ? `stores:active:${scopeKey}:${storeGroup}`
      : `stores:active:${scopeKey}`;
    const cached = appCache.get(cacheKey);
    if (cached) return res.json(cached);

    const stores = storeGroup
      ? await db.getStoresByFilters({ storeGroup })
      : await db.getStoresForAssignment();

    const visible = scope.canViewAll
      ? stores
      : stores.filter(s => s.store_group && scope.groups.includes(s.store_group));

    const result = visible.map(safeStore);
    appCache.set(cacheKey, result);
    res.json(result);
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.get('/api/stores/all', authenticateToken, async (req, res) => {
  try {
    ...
    if (cached) return res.json(cached);
    const result = await db.pool.query('SELECT * FROM stores ORDER BY brand_name ASC');
    const stores = result.rows.map(safeStore);
    appCache.set('stores:all', stores);
    res.json(stores);
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.post('/api/stores', authenticateToken, async (req, res) => {
  try {
    ...
    appCache.invalidatePrefix('stores:active');
    appCache.invalidate('stores:all');
    res.status(201).json(safeStore(result.rows[0]));
  } catch (error) {
    if (error.code === '23505') return res.status(409).json({ error: 'A store with that identifier or domain already exists' });
    res.status(500).json({ error: error.message });
  }
...
app.put('/api/stores/:id', authenticateToken, async (req, res) => {
  try {
    ...
    invalidateStoreCache(result.rows[0].store_identifier);
    invalidateStoreCache(result.rows[0].shop_domain);
    res.json(safeStore(result.rows[0]));
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.delete('/api/stores/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Group assignments survive a store deletion untouched — that's the point
    // of assigning groups rather than store ids.
    res.json({ success: true });
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.get('/api/conversations', authenticateToken, async (req, res) => {
  try {
    ...
    // Permission scope. Set as an array even when empty — db.getConversations
    // checks Array.isArray, so a restricted agent with no assignments correctly
    // sees nothing instead of everything.
    const scope = await getRequestStoreScope(req);
    if (!scope.canViewAll) filters.storeGroups = scope.groups;

    // NOTE: the date bounds MUST be part of the key — otherwise a date-filtered
    // request and an unfiltered one collide and serve each other's rows. The
    // scope segment matters for the same reason: without it a restricted
    // agent's 12-row result is served to an admin, and the admin's full list is
    // served to the agent.
    const cacheKey = `convs:${scopeCacheKey(scope, req)}:${storeId || 'all'}:${status || 'open'}:${limit || 'def'}:${offset || '0'}:${storeGroup || 'allgroups'}:${dateFrom || 'any'}:${dateTo || 'any'}`;
    const cached = appCache.get(cacheKey);
    if (cached) return res.json(cached);
...
app.get('/api/conversations/linked/:email', authenticateToken, async (req, res) => {
  try {
    ...
    const scope = await getRequestStoreScope(req);
    const cacheKey = `linked:${scopeCacheKey(scope, req)}:${email}:${excludeConversationId || 'none'}`;
    const cached = appCache.get(cacheKey);
    if (cached) return res.json(cached);

    const params = excludeConversationId ? [email, parseInt(excludeConversationId, 10)] : [email];
    const scopeClause = scopeSqlClause(scope, params);
    const result = await db.pool.query(`
      SELECT c.id, c.status, c.created_at, c.updated_at, c.shop_domain, c.shop_id,
             COALESCE(s.brand_name, c.shop_domain, 'Unknown Store') AS brand_name,
             m.content AS last_message_content, m.sender_type AS last_message_sender_type,
       ...
       WHERE c.customer_email = $1 ${excludeConversationId ? 'AND c.id != $2' : ''}${scopeClause}
       ORDER BY c.updated_at DESC LIMIT 200
    `, params);
...
app.get('/api/conversations/search', authenticateToken, async (req, res) => {
  try {
    ...
    // Permission scope — search must not become a way around the store filter.
    const userScope = await getRequestStoreScope(req);
    if (!userScope.canViewAll) {
      params.push(userScope.groups);
      scope.push(`c.shop_id IN (SELECT id FROM stores WHERE store_group = ANY($${params.length}::text[]))`);
    }
...
app.get('/api/conversations/archived', authenticateToken, async (req, res) => {
  try {
    ...
    const scope = await getRequestStoreScope(req);
    whereExtra += scopeSqlClause(scope, params);
...
app.put('/api/conversations/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const conversation = await db.updateConversation(conversationId, req.body);
    if (!conversation) return res.status(404).json({ error: 'Conversation not found' });
    appCache.invalidatePrefix('convs:');
    res.json(snakeToCamel(conversation));
  } catch (error) { res.status(500).json({ error: error.message }); }
...
app.put('/api/conversations/:id/close', authenticateToken, async (req, res) => {
  try {
    ...
    const conversation = await db.closeConversation(conversationId);
    if (!conversation) return res.status(404).json({ error: 'Conversation not found' });
    appCache.invalidatePrefix('convs:');
    res.json(snakeToCamel(conversation));
  } catch (error) { res.status(500).json({ error: error.message }); }
...
app.post('/api/messages', authenticateToken, async (req, res) => {
  try {
    ...
    // Checked BEFORE the optimistic broadcast — otherwise a restricted agent's
    // rejected message still appears in every other agent's UI.
    if (!(await canAccessConversation(req, parseInt(conversationId, 10)))) {
      return res.status(403).json({ error: 'You do not have access to this conversation' });
    }
...
app.post('/api/blacklist', authenticateToken, async (req, res) => {
  ...
  const blockedBy = req.user?.name || req.user?.email || null;
  try {
    // A network-wide block reaches every store in the fleet, so it stays an
    // admin action. A restricted agent can still block on their own stores.
    const scope = await getRequestStoreScope(req);
    if (allStores && !scope.canViewAll) {
      return res.status(403).json({ error: 'Network-wide blacklisting requires full store access' });
    }
...
app.delete('/api/blacklist/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // Lifting a network-wide block is the mirror of applying one.
    const scope = await getRequestStoreScope(req);
    if (!store_identifier && !scope.canViewAll) {
      return res.status(403).json({ error: 'Removing a network-wide block requires full store access' });
    }
...
// The admin UI sends and reads `assignedGroups`. snakeToCamel already produces
// that from assigned_groups, but the alias is set explicitly so a NULL column
// arrives as [] rather than null — the picker would otherwise crash on .map.
// legacy assigned_stores is deliberately NOT exposed; permissions are per-group.
function withStoreAssignment(employeeRow, camelRow) {
  const { assignedStores, ...rest } = camelRow;
  return {
    ...rest,
    assignedGroups: employeeRow.assigned_groups || [],
  };
}

/**
 * Normalise the group assignment out of a request body. Accepts either
 * assignedGroups (what the UI sends) or assignedStoreGroups, and returns
 * undefined when neither was supplied so a partial update doesn't wipe them.
 */
function readAssignedGroups(body) {
  const raw = body.assignedGroups !== undefined ? body.assignedGroups
            : body.assignedStoreGroups !== undefined ? body.assignedStoreGroups
            : undefined;
...
app.get('/api/employees', authenticateToken, async (req, res) => {
  try {
    ...
    const enriched = employees.map(emp => {
      const { password_hash, api_token, ...safe } = emp;
      return {
        ...withStoreAssignment(emp, snakeToCamel(safe)),
        ...(statsById[String(emp.id)] || { avgResponseMinutes: null, fastestMinutes: null, totalResponsesCounted: 0 }),
        responsesByCustomer: responsesByAgent[String(emp.id)] || [],
      };
    });
...
app.post('/api/employees', authenticateToken, async (req, res) => {
  try {
    ...
    const canViewAll = canViewAllStores !== undefined ? canViewAllStores : true;
    const assignedGroups = readAssignedGroups(req.body) ?? [];

    // A restricted account with no groups sees an empty dashboard, which reads
    // as a broken login rather than a permissions decision.
    if (!canViewAll && assignedGroups.length === 0) {
      return res.status(400).json({ error: 'Select at least one store group, or enable full store access' });
    }

    // Reject slugs that don't exist — a typo'd group would save cleanly and
    // then silently grant access to nothing.
    if (!canViewAll) {
      const { rows: known } = await db.pool.query(
        `SELECT group_key FROM store_groups WHERE group_key = ANY($1::text[])`, [assignedGroups]);
      const missing = assignedGroups.filter(g => !known.some(k => k.group_key === g));
      if (missing.length) {
        return res.status(400).json({ error: `Unknown store group(s): ${missing.join(', ')}` });
      }
    }

    const password_hash = await hashPassword(password);
    const saved = await db.createEmployee({
      email, name, employee_name: employeeName || null, role: role || 'agent', password_hash,
      can_view_all_stores: canViewAll,
      is_active: isActive !== undefined ? isActive : true,
      assigned_groups: canViewAll ? [] : assignedGroups,
    });

    appCache.invalidate('employees:list');
    appCache.invalidatePrefix('scope:');
    const row = { ...saved };
    delete row.password_hash; delete row.api_token;
    res.json(withStoreAssignment(row, snakeToCamel(row)));
  } catch (error) {
    if (error.code === '23505') return res.status(409).json({ error: 'An employee with that email already exists' });
...
app.put('/api/employees/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const assignedGroups = readAssignedGroups(updates);
    if (assignedGroups !== undefined) dbUpdates.assigned_groups = assignedGroups;

    // Same guard as create, but only when this request is actually narrowing
    // access — a status-only PATCH shouldn't be rejected for a field it didn't
    // touch.
    if (dbUpdates.can_view_all_stores === false) {
      const effective = assignedGroups !== undefined
        ? assignedGroups
        : (await db.getEmployeeStoreScope(employeeId)).groups;
      if (!effective || effective.length === 0) {
        return res.status(400).json({ error: 'Select at least one store group, or enable full store access' });
      }
    }

    if (assignedGroups !== undefined && assignedGroups.length) {
      const { rows: known } = await db.pool.query(
        `SELECT group_key FROM store_groups WHERE group_key = ANY($1::text[])`, [assignedGroups]);
      const missing = assignedGroups.filter(g => !known.some(k => k.group_key === g));
      if (missing.length) {
    ...
    appCache.invalidate('employees:list');
    appCache.invalidatePrefix('auth:');
    // Their visible conversation set just changed — both the scope lookup and
    // every conversation page cached under it are now stale.
    appCache.invalidate(`scope:${employeeId}`);
    appCache.invalidatePrefix(`convs:emp${employeeId}:`);
    appCache.invalidatePrefix(`stores:active:emp${employeeId}`);
