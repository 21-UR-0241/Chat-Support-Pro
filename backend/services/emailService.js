...
// TUNABLES
// ──────────────────────────────────────────────
// parseInt on a typo'd env var yields NaN, which silently turns COOLDOWN_MS into
// NaN (every comparison false → cooldown never applies) and breaks the sweep's
// INTERVAL literal. toInt() falls back instead of poisoning the maths.
...
// ──────────────────────────────────────────────
// SHARED JSON-MAP ENV PARSING
// ──────────────────────────────────────────────
// Three separate per-group maps now use this shape. Malformed JSON is a config
// error worth shouting about, but it must not stop the module from loading.
...
// ──────────────────────────────────────────────
// PER-GROUP RESEND CLIENTS
// ──────────────────────────────────────────────
// Each store group can send from its own Resend account, so a suspended or
// rate-limited key on one group never takes down email for the others (and
// each group's sending domain/reputation stays isolated).
//
// Key lookup order for a store in group `peptides-group`:
//   1. RESEND_API_KEY_PEPTIDES_GROUP     ← group-specific (slug uppercased, non-alphanumerics → _)
//   2. RESEND_KEYS_BY_GROUP              ← optional JSON map, e.g. {"peptides-group":"re_xxx"}
//   3. RESEND_API_KEY                    ← shared default
//
// Clients are cached by API key (not by group), so several groups pointing at
// the same key share one client instead of allocating a duplicate each.
...
function resolveApiKey(storeGroup) {
  if (storeGroup) {
    ...
    // Group known but unconfigured — fall back, and say so once per group so a
    // missing key surfaces in logs instead of silently sending from the default.
...
/**
 * Resend client for a store group. Returns null when no key is configured at
 * all, so callers can skip the send rather than throw deep inside the SDK.
 */
...
// ──────────────────────────────────────────────
// PER-GROUP SENDER ADDRESSES
// ──────────────────────────────────────────────
// The from-domain MUST be verified in the SAME Resend account as that group's
// API key — otherwise the send is rejected, or (worse) accepted and filed as
// spam because SPF/DKIM can't validate. A single global fallback address only
// works while every group shares one key; once keys are split per group, the
// fallback has to be split too.
//
// Resolution order for a store's sender address:
//   1. stores.email_from_address     ← per-store override (wins if set)
//   2. RESEND_FROM_BY_GROUP[group]   ← per-group fallback, JSON map
//   3. FALLBACK_FROM_ADDRESS         ← global last resort
//
// Config example:
//   RESEND_FROM_BY_GROUP={"lexar-peptides":"support@lexardomain.com","sex-pills":"support@spdomain.com"}
//
// NOTE: stores.email_from_address is shared with promo-routes.js and the
// server.js sender. Changing that column moves the FROM for those flows too.
// Prefer RESEND_FROM_BY_GROUP when you only want the chat emails to change.
...
function resolveFromAddress(storeGroup, storeAddress) {
  ...
  if (storeGroup) {
    ...
    // No per-group sender configured. This is the failure mode worth shouting
    // about: the group may resolve its OWN Resend key while still sending from
    // the shared fallback domain, which that account almost certainly hasn't
    // verified — Resend will reject it, or deliver it straight to spam.
...
/**
 * Where customer replies land. Without this, a customer hitting "Reply" is
 * writing to the no-reply sending address and the message is lost.
 *
 * Config: RESEND_REPLY_TO_BY_GROUP={"lexar-peptides":"help@lexardomain.com"}
 * Falls back to RESEND_REPLY_TO, then to the from-address.
 */
...
// ──────────────────────────────────────────────
// Bounces damage sender reputation, which is the same reputation deciding
// whether the next email lands in inbox or spam. Addresses that cannot possibly
// deliver are dropped before they reach Resend.
...
/**
 * Returns null when the address looks deliverable, or a human-readable reason
 * when it does not.
 */
...
/**
 * RFC 5322 display names containing commas, quotes, angle brackets etc. break
 * the From header unless quoted. A brand name like `Lab, Inc.` would otherwise
 * be parsed as two addresses.
 */
...
/**
 * Logs, once at startup, exactly what every store group will resolve to. This
 * exists because a group can resolve its own API key while silently falling
 * back to the shared from-address — a mismatch that only shows up as a spam
 * placement or a bounce hours later. It also makes a dev/prod env drift
 * obvious the moment the service boots.
 */
...
async function sendOfflineEmail(pool, conversation_id, triggerMessageId) {
  try {
    ...
    // 3. Get conversation + store email config
    //    Get customer_email from conversations table (reliable source)
    //    store_group drives which Resend account this email sends from.
    const convResult = await pool.query(
      `SELECT 
         c.id,
         c.customer_email,
    ...
    // Undeliverable addresses are dropped here rather than bounced at Resend —
    // bounce rate feeds directly into inbox-vs-spam placement for every other
    // email this domain sends. Warn once per conversation so the sweep retrying
    // every 5 minutes doesn't flood the log.
    ...
    // Which group this store belongs to — drives BOTH the Resend account used
    // and the sender-address fallback. Resolved before the config block so the
    // from-address can key off it.
    const storeGroup = conv.store_group || null;

    // Email config. brand_name is NOT NULL-guarded in the schema, and a null or
    // single-character value used to produce a broken From header and a crash
    // inside the initials builder.
    ...
    // Pick the Resend account for this store's group. Bail before doing the
    // message work if nothing is configured — better a clear log line than a
    // cryptic SDK auth error.
    ...
    if (!unreadMessages.length) {
      // This used to return silently, which looked identical to a crash when
      // debugging. The cutoff is the usual culprit: a heartbeat bumping
      // last_activity_at past the agent's message makes it "already read".
 ...
 *   stopEmailSweep();
 *
 * NOTE: pendingSends is per-process. If this service ever runs on more than one
 * Render instance, two processes can debounce the same conversation
 * independently. The cooldown check limits the damage to one duplicate, but a
 * unique index on offline_email_log (conversation_id, message_id) would close
 * it properly.
 */
function startEmailSweep(pool) {
  console.log(`[Email Sweep] Started — checking every ${SWEEP_INTERVAL_MS / 1000}s`);
...
async function runSweep(pool) {
  try {
    // Find all conversations with unsent agent messages where customer is offline.
    // COOLDOWN_HOURS is passed as a parameter rather than interpolated into the
    // SQL string, so a bad env value can't produce a broken INTERVAL literal.
    const result = await pool.query(`
      WITH last_emailed AS (
        SELECT 
          oel.conversation_id,
