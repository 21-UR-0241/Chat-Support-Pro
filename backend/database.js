...
const pool = new Pool({
  ...
  statement_timeout: 15000,        // ← NEW: kill any runaway query so it can't pin a connection
...
async function runMigrations() {
  ...
  try {
    ...
    await migration_017_add_promo_tables();
    await migration_018_add_performance_indexes();   // ← NEW: hot-path indexes
    await migration_019_add_response_stats_rollup();  // ← NEW: response-time rollup tables
    await migration_020_add_group_columns(); 
    await migration_021_add_store_groups_table();
    await migration_022_add_group_color();
...
async function migration_016_add_employee_name() {
...
// ============================================
// MIGRATION 018 — Hot-path performance indexes
// ============================================
// These are the indexes the dashboard / message / notes queries depend on.
//
// ⚠️  NOTE ON LARGE TABLES: plain CREATE INDEX takes an ACCESS EXCLUSIVE lock
//     and blocks writes to `messages` while it builds. On a very large messages
//     table this can stall the app at boot for a few seconds. If that is a
//     concern, run these ONCE manually during low traffic using
//     CREATE INDEX CONCURRENTLY (cannot run inside a transaction), e.g.:
//
//       CREATE INDEX CONCURRENTLY idx_messages_conv_customer_lastmsg
//         ON messages (conversation_id, id DESC) WHERE sender_type = 'customer';
//
//     After they exist, this migration becomes a no-op (IF NOT EXISTS).
// ============================================
async function migration_018_add_performance_indexes() {
  const migrationName = 'Migration 018';
  try {
    console.log(`📝 [${migrationName}] Ensuring hot-path performance indexes...`);
    const statements = [
      // getConversations() — makes the per-row "last customer message" subquery an index lookup
      `CREATE INDEX IF NOT EXISTS idx_messages_conv_customer_lastmsg
         ON messages (conversation_id, id DESC) WHERE sender_type = 'customer'`,

      // getMessages() — keyset pagination on (conversation_id, id)
      `CREATE INDEX IF NOT EXISTS idx_messages_conv_id
         ON messages (conversation_id, id DESC)`,

      // markConversationRead() — the "unread customer messages" UPDATE
      ...
      // getDashboardStats() — "messages today" count (timestamp is sargable)
      `CREATE INDEX IF NOT EXISTS idx_messages_timestamp_shop
         ON messages (timestamp DESC, shop_id)`,

      // response-time window scans (used by the rollup + any live stats)
    ...
    for (const sql of statements) {
      try { await pool.query(sql); created++; }
      catch (e) { console.warn(`   [${migrationName}] Skipped index: ${e.message.substring(0, 90)}`); }
    }
    console.log(`✅ [${migrationName}] ${created}/${statements.length} performance indexes ensured`);
  } catch (error) {
    console.error(`❌ [${migrationName}] Failed:`, error);
    throw error;
  }
...
// ============================================
// MIGRATION 019 — Response-time rollup tables
// ============================================
// Precomputed agent response-time stats so the /api/employees page no longer
// runs two full-table window-function scans on every request. Populate these
// with refreshResponseStats() on a background interval (see server.js wiring
// note in the chat). Empty/unused tables are harmless if not yet wired.
// ============================================
async function migration_019_add_response_stats_rollup() {
  const migrationName = 'Migration 019';
  try {
    console.log(`📝 [${migrationName}] Creating response-time rollup tables...`);
    await pool.query(`
    ...
    `);
    console.log(`✅ [${migrationName}] Response-stats rollup tables ready`);
  } catch (error) {
    console.error(`❌ [${migrationName}] Failed:`, error);
    throw error;
  }
...
// ============================================
// MIGRATION 020 — store_group column (dashboard bg color grouping)
// ============================================
async function migration_020_add_group_columns() {
  const migrationName = 'Migration 020';
  try {
    console.log(`📝 [${migrationName}] Adding store_group and store_group_name columns to stores...`);
    await pool.query(`ALTER TABLE stores ADD COLUMN IF NOT EXISTS store_group VARCHAR(100) DEFAULT NULL;`);
    await pool.query(`ALTER TABLE stores ADD COLUMN IF NOT EXISTS store_group_name VARCHAR(150) DEFAULT NULL;`);
    await pool.query(`
      CREATE INDEX IF NOT EXISTS idx_stores_store_group ON stores(store_group) WHERE store_group IS NOT NULL;
    `);
    console.log(`✅ [${migrationName}] store_group and store_group_name columns added`);
  } catch (error) {
    console.error(`❌ [${migrationName}] Failed:`, error);
    throw error;
  }
...
// ============================================
// MIGRATION 021 — store_groups registry table
// ============================================
// Groups previously only "existed" implicitly via GROUP BY on stores.store_group,
// so an empty/not-yet-used group had nowhere to live. This table lets admins
// pre-create a group before any store is assigned to it.
async function migration_021_add_store_groups_table() {
  const migrationName = 'Migration 021';
  try {
    console.log(`📝 [${migrationName}] Creating store_groups table...`);
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_store_groups_key ON store_groups(group_key);`);

    // One-time backfill: register whatever group_key values are already in use
    // on stores so existing groups (e.g. peptides-group) don't "disappear"
    // from the registry just because this table is new.
    const backfill = await pool.query(`
      INSERT INTO store_groups (group_key, group_name)
      SELECT DISTINCT ON (store_group)
        store_group,
        COALESCE(store_group_name, store_group)
      FROM stores
      WHERE store_group IS NOT NULL
      ORDER BY store_group, updated_at DESC
      ON CONFLICT (group_key) DO NOTHING
    `);
    console.log(`✅ [${migrationName}] store_groups table ready, backfilled ${backfill.rowCount} group(s)`);
  } catch (error) {
    console.error(`❌ [${migrationName}] Failed:`, error);
    throw error;
  }
...
// ============================================
// MIGRATION 022 — color column on store_groups
// ============================================
async function migration_022_add_group_color() {
  const migrationName = 'Migration 022';
  try {
    console.log(`📝 [${migrationName}] Adding color column to store_groups...`);
    await pool.query(`ALTER TABLE store_groups ADD COLUMN IF NOT EXISTS color VARCHAR(7) DEFAULT '#667eea';`);
    console.log(`✅ [${migrationName}] color column added`);
  } catch (error) {
    console.error(`❌ [${migrationName}] Failed:`, error);
    throw error;
  }
...
async function registerStore(storeData) {
  ...
  try {
    ...
    `, [
      store_identifier, shop_domain, brand_name, access_token, api_key,
      api_secret, scope, timezone, currency, logo_url, primary_color,
      contact_email, store_tags, store_group, store_group_name
    ]);
    return result.rows[0];
  } catch (error) {
    console.error('Error registering store:', error);
...
async function getStoresByFilters(filters = {}) {
  try {
    ...
    const params = [];
    let paramCount = 1;
    if (filters.storeGroup) { query += ` AND store_group = $${paramCount}`; params.push(filters.storeGroup); paramCount++; }
...
async function getConversations(filters = {}) {
  try {
    ...
    if (filters.storeId)         { query += ` AND c.shop_id = $${paramCount}`; params.push(filters.storeId); paramCount++; }
    if (filters.storeIdentifier) { query += ` AND c.shop_domain = $${paramCount}`; params.push(filters.storeIdentifier); paramCount++; }
    if (filters.storeGroup)      { query += ` AND s.store_group = $${paramCount}`; params.push(filters.storeGroup); paramCount++; }
    if (filters.customerEmail)   { query += ` AND c.customer_email = $${paramCount}`; params.push(filters.customerEmail); paramCount++; }
    if (filters.status)          { query += ` AND c.status = $${paramCount}`; params.push(filters.status); paramCount++; }
    if (!filters.status && filters.excludeArchived) query += ` AND c.status != 'archived'`;
    if (filters.priority)        { query += ` AND c.priority = $${paramCount}`; params.push(filters.priority); paramCount++; }
    ...
    // Default cap so an unbounded fetch can't return the whole table (max 100).
...
//   try {
//     const result = await pool.query(`
//       SELECT sg.id, sg.group_key, sg.group_name, sg.created_at, sg.updated_at,
...
//       GROUP BY sg.id, sg.group_key, sg.group_name, sg.created_at, sg.updated_at
...
// async function createStoreGroup({ group_key, group_name }) {
//   try {
//     const result = await pool.query(`
//       INSERT INTO store_groups (group_key, group_name, created_at, updated_at)
...
// // Renaming the key cascades to every store already using the old key —
// // done inside a transaction so the registry and the stores table can't
// // drift apart if the second UPDATE fails.
// async function updateStoreGroup(id, { group_key, group_name }) {
//   const client = await pool.connect();
//   try {
...
//       UPDATE store_groups SET group_key = $1, group_name = $2, updated_at = NOW()
//       WHERE id = $3 RETURNING *
//     `, [group_key, group_name, id]);

//     if (group_key !== oldKey) {
//       await client.query(
//         `UPDATE stores SET store_group = $1, updated_at = NOW() WHERE store_group = $2`,
//         [group_key, oldKey]
//       );
//     }
...
// force=false (default): refuses to delete a group that still has stores.
// force=true: unassigns the group from those stores first, then deletes it.
async function deleteStoreGroup(id, { force = false } = {}) {
  try {
    const existing = await pool.query('SELECT * FROM store_groups WHERE id = $1', [id]);
    if (!existing.rows[0]) return { deleted: false, reason: 'not_found' };
    const { group_key } = existing.rows[0];

    const countResult = await pool.query('SELECT COUNT(*)::int AS n FROM stores WHERE store_group = $1', [group_key]);
    const storeCount = countResult.rows[0].n;

    if (storeCount > 0 && !force) return { deleted: false, reason: 'has_stores', storeCount };

    if (storeCount > 0 && force) {
      await pool.query(
        'UPDATE stores SET store_group = NULL, store_group_name = NULL, updated_at = NOW() WHERE store_group = $1',
        [group_key]
      );
    }
    await pool.query('DELETE FROM store_groups WHERE id = $1', [id]);
...
// Backward compatible:
//   getMessages(conversationId)                       → full history, oldest→newest (UNCHANGED)
//   getMessages(conversationId, { limit })            → newest `limit`, oldest→newest
//   getMessages(conversationId, { limit, before })    → page older than cursor id `before`
// The expensive regex/cast agent-name join now runs on at most `limit` rows
// when paginating, instead of the entire thread.
  ...
  try {
    ...
    if (limit) {
      ...
      const result = await pool.query(
        `SELECT * FROM (
           SELECT m.*,
                  e.name          AS sender_display_name,
                  e.employee_name AS sender_employee_name
             FROM messages m
             ${joinClause}
    ...
    // ── default: unchanged full-history behaviour ──
    const result = await pool.query(
      `SELECT m.*,
              e.name          AS sender_display_name,
              e.employee_name AS sender_employee_name
...
async function refreshResponseStats() {
  // ── per-agent rollup ──
  await pool.query(`
    INSERT INTO agent_response_stats (sender_id, avg_response_minutes, fastest_minutes, total_responses_counted, updated_at)
  ...
  // ── per-(agent, customer) rollup — full recompute in a txn ──
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query('DELETE FROM agent_customer_response_stats');
    await client.query(`
      INSERT INTO agent_customer_response_stats (sender_id, customer_email, avg_minutes, response_count)
      WITH real_messages AS (
  ...
  console.log('📊 [Stats] Response-time rollup refreshed');
}
...
async function getAgentCustomerResponseStats() {
  const { rows } = await pool.query(
    `SELECT * FROM agent_customer_response_stats ORDER BY sender_id, response_count DESC`
  );
  const byAgent = {};
  for (const r of rows) {
...
// OPTIMISED — was LEFT JOIN-ing the entire messages table against conversations
// and filtering with DATE(m.timestamp) (non-sargable). Now: conversation
// aggregates run alone, and "messages today" is a separate sargable count that
// uses idx_messages_timestamp_shop. Same return keys as before.
//
// Minor semantic note: messages_today now counts ALL messages sent today
// (including those in archived conversations). The old version excluded
// archived conversations' messages. The difference is negligible in practice
// (you rarely archive same-day high-volume threads). If you need the exact old
// semantics back, re-add the join — but that reintroduces the slow path.
async function getDashboardStats(filters = {}) {
  try {
    const params = [];
    let storeFilter = '';
    if (filters.storeId) { storeFilter = 'AND shop_id = $1'; params.push(filters.storeId); }
    ...
    const convStats = await pool.query(`
      ...
      FROM conversations
      WHERE status != 'archived' ${storeFilter}
    `, params);

    const msgStats = await pool.query(
      `SELECT COUNT(*) AS messages_today
         FROM messages
        WHERE timestamp >= CURRENT_DATE ${storeFilter}`,
      params
    );
