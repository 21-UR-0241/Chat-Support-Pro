...
// NOTE: behaviour unchanged — still returns the full matching set when no limit
// is passed, so the (non-paginated) frontend keeps working. The per-row
// "last customer message" subquery is now an index lookup thanks to
// idx_messages_conv_customer_lastmsg (migration 018). Pass filters.limit /
// filters.offset once the frontend supports pagination.
async function getConversations(filters = {}) {
  try {
    let query = `
      SELECT c.*, s.brand_name, s.logo_url, s.primary_color, s.store_identifier,
